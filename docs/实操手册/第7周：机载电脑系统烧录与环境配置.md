...
|<a href="https://pan.baidu.com/s/1l1JZ2JmJGVTB1fKGJFRWSg?pwd=diff" target="_blank">📥orangepi.img</a>|orangepi.img|香橙派镜像文件|
...
1. 下载课程中提供的`orangepi.img`镜像

2. 对下载的镜像压缩包进行解压，提取其中的系统镜像文件（\.img格式）
...
1. 下载解压工具‵RKDevTool`，并对驱动压缩包进行解压，提取其中的驱动文件
...
<div style="background-color:#edf2ff; border:1px solid #88aaff; padding:20px 26px; margin:16px 0; border-radius:14px; color:#222; line-height:2.4;">
  💡 连接香橙派前，要将香橙派底板与核心板组装完成，单独底板无法进行烧录。
</div>
...
3. 移除风扇接口，按住开发板最左边按键，usb线连接电脑与香橙派，等待绿灯亮起后松手，此时烧录工具会显示发现一个 MASKROM 设备
...
<div style="display:flex; gap:16px; width:100%; box-sizing:border-box;">
  ...
  <div style="flex:1; background:#ffffff; border-radius:16px; padding:12px; border:1px solid #eee;">
    <img src="../images/week_7/7-5.jpg" style="width:100%; height:auto; display:block;">
    <div style="text-align:center; font-size:1.4em; margin-top:12px; color:#333;">Loader文件选择</div>
  </div>

  <!-- 卡片2：MASKROM设备识别成功 -->
  <div style="flex:1; background:#ffffff; border-radius:16px; padding:12px; border:1px solid #eee;">
    <img src="../images/week_7/7-6.png" style="width:90%; height:auto; display:block;">
    <div style="text-align:center; font-size:1.4em; margin-top:12px; color:#333;">MASKROM设备识别成功</div>
  </div>

  <!-- 卡片3：系统镜像文件选择 -->
  <div style="flex:1; background:#ffffff; border-radius:16px; padding:12px; border:1px solid #eee;">
    <img src="../images/week_7/7-7.png" style="width:93%; height:auto; display:block;">
...
<div style="background-color:#ffffe6; border:1px solid #f8dd47; padding:20px 26px; margin:16px 0; border-radius:14px; color:#222; line-height:2.4;">
  💡 <strong>注意：</strong>端口号需要根据实际连接的串口设备进行配置，可以使用 <code style="background:#eee;padding:2px 8px;border-radius:4px;">ls /dev/ttyUSB*</code> 或 <code style="background:#eee;padding:2px 8px;border-radius:4px;">ls /dev/ttyACM*</code> 查看设备名称。
</div>
